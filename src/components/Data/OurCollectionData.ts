import GranEspresso from '../../assets/images/home/desktop/image-gran-espresso.png';
import Planalto from '../../assets/images/home/desktop/image-planalto.png';
import Piccollo from '../../assets/images/home/desktop/image-piccollo.png';
import Danche from '../../assets/images/home/desktop/image-danche.png';

export const OurCollectionData = [
    {
        style: 'w-[200px] md:w-[300px]  lg:w-[250px] text-center flex flex-col items-center justify-center my-7 xl:my-5  z-1000' ,
        space: false,
        imgSpace: 'w-[150px] md:w-[170px]  lg:w-[200px] my-2 xl:pb-20 xl:my-5',
        icon:GranEspresso,
        title:'Gran Espress',
        description: 'Light and flavorful blend with cocoa and black pepper for an intense experience'
    },
    {
         style: 'w-[200px] md:w-[300px]  lg:w-[250px] text-center flex flex-col items-center justify-center my-7 xl:my-5  z-1000 ',
        space: false,
        imgSpace: 'w-[150px]  md:w-[170px] lg:w-[200px] my-2 xl:pb-20 xl:my-5',
        icon:Planalto,
        title:'Planalto',
        description: 'Brazilian dark roast with rich and velvety body, and hints of fruits and nuts'
    },
    {
         style: 'w-[200px] md:w-[300px]  lg:w-[250px] text-center flex flex-col items-center justify-center my-7 xl:my-5  z-1000 ',
        space: false,
        imgSpace: 'w-[150px]  md:w-[170px] lg:w-[200px] my-2 xl:pb-20 xl:my-5',
        icon:Piccollo,
        title:'Piccollo',
        description: 'Mild and smooth blend featuring notes of toasted almond and dried cherry'
    },
    {
         style: 'w-[200px] md:w-[300px]  lg:w-[250px] text-center flex flex-col items-center justify-center my-7 xl:my-5  z-1000 ',
        space: true,
        imgSpace: 'w-[150px]  md:w-[170px] lg:w-[200px] my-2 xl:pb-20 xl:my-5',
        icon:Danche,
        title:'Danche',
        description: 'Ethiopian hand-harvested blend densely packed with vibrant fruit notes'
    }
]